...
拍一张外语菜单的照片，得到一份**同版式**的中文对照 PDF：

- 📷 **输入**：菜单照片（拍照或从相册选择）
- 🧠 **识别**：调用 OpenAI 视觉模型（默认 `gpt-4.1`），一次请求完成
  OCR、版式提取（归一化 bounding box）、整句翻译和**逐词对照**（interlinear gloss，
  含日语假名读音等 romanization）
- 🖼️ **配图**：如果菜单本身印有菜品照片，模型会返回照片的 bbox，App 直接从
  原图裁剪出来 —— 不联网搜图、不用生成模型
- 📄 **输出**：一份大尺寸 PDF
  - **版式页**：与照片同比例的大页面，原图淡淡地垫在底下，每道菜的双语卡片
    画在它自己的 bbox 位置上 —— 读起来就像原菜单，只是变成了双语
  - **附录页**：每道菜一张可读性优先的卡片（裁剪配图 + 完整逐词对照表），分页排版
